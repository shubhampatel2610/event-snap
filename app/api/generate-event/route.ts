    ...
    try {
        ...
        if (!prompt) {
            return NextResponse.json(
                { error: AppConstants.PROMPT_REQUIRED_ERROR },
                { status: 400 }
            );
        }

        const aiModel = genAI.getGenerativeModel({ model: AppConstants.GEMINI_AI_MODEL });

        const systemPromptForEvent = `You are an event planning assistant. Generate event details based on the user's description.
        ...
        const result = await aiModel.generateContent(systemPromptForEvent);
        ...
        return NextResponse.json(
            { error: `${AppConstants.GENERATE_EVENT_ERROR}: ${error.message}` },
            { status: 500 }
        );
    }
}